...
class HTTPClient:
    """Send one HTTP request and receive one HTTP response."""
    ...
    def _read(self):
        ...
        if data:
            ...
            raise RuntimeError("Peer closed before sending a complete response.")
    ...
    def process_response(self):
        ...
        version, status_code, reason = status_line
        if not version.startswith("HTTP/"):
            raise ValueError("unsupported HTTP response version")
        ...
        try:
            ...
            raise ValueError("response content-length must be an integer") from error
        if content_length < 0:
            raise ValueError("response content-length cannot be negative")
        if len(remainder) < content_length:
            return

        body = remainder[:content_length]
        self._recv_buffer = remainder[content_length:]
        self.response = {
            "version": version,
